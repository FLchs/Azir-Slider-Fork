...
const AzirSlider: React.FC<Props> = (props) => {
  ...
  const thumbTouchSize = _thumbTouchSize
    ? _thumbTouchSize
    : { width: 20 + __thumbSize, height: 20 + __thumbSize };
  ...
  const _getRatio = (__value: number) =>
    (__value - minimumValue) / (maximumValue - minimumValue);
  ...
  return (
    <View
      {...rest}
      style={[{ height: __thumbSize }, styles.container, style]}
      onLayout={_measureContainer}>
      <View
        style={[
          {
            backgroundColor: getColorByName(trackColor, theme.COLORS),
            height: _trackSize ? _trackSize : theme.SIZES.TRACK_SIZE,
        ...
        style={[
          {
            ...
            borderRadius: __thumbSize
              ? __thumbSize
              : theme.SIZES.THUMB_SIZE / 2,
          },
