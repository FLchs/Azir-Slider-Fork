...
export function withAzir(Component: React.ElementType, styles: AzStyle) {
  return class extends React.Component {
    render() {
      const { props } = this;
      return (
        <AzirContext.Consumer>
          {(theme) => (
            <Component
              {...props}
              theme={{ ...AzirTheme, ...theme }}
              styles={styles && styles({ ...AzirTheme, ...theme })}
            />
          )}
        </AzirContext.Consumer>
      );
...
export const AzirProvider: React.FC<Props> = (props) => {
  ...
  return (
    <AzirContext.Provider value={providerTheme}>
      {children}
    </AzirContext.Provider>
  );
};
...
type Props = {
  children: unknown;
  theme: Record<string, Record<string, number>>;
};
//default props 4
...
export const AzirGridContext = React.createContext({
  width: 0,
  height: 0,
  spacing: 0,
});
